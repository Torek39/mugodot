...
func apply_state():
	...
	if Global.barrier_opened:
		...
		if npc and npc.has_method("set_solved"):
			npc.set_solved()
	if Global.bridge_repaired:
		var bridge = _find(scene, "bridge_incomplete")
		if bridge and bridge.has_method("repair"):
			bridge.repair()
	if Global.wall_detonated:
		var wall = _find(scene, "explosion_wall")
		if wall and wall.has_method("detonate"):
			wall.wall1.modulate.a = 0.0
			wall.wall1.collision_enabled = false
			wall.wall2.modulate.a = 0.0
			wall.wall2.collision_enabled = false
	if Global.guardian_chest_opened:
	# Сундук открыт
		var chest = _find(scene, "treasure_chest")
		if chest and chest.has_method("open"):
			chest.open()
	
	# Голем готов, препятствие убрано
	var golem = _find(scene, "golem_guardian")
	if golem and golem.has_method("_remove_obstacle_immediately"):
		golem._remove_obstacle_immediately()
	
	# Ставим флаг что сундук уже открыт, чтобы голем не показывал стартовые диалоги
	if golem:
		golem.is_chest_opened = true
		
	if Global.teleport_activated:
		var device = _find(scene, "teleport_device")
		if device and device.has_method("activate"):
			device.activate()
	if Global.glitch_fixed:
		var corridor = _find(scene, "glitch_corridor")
		if corridor and corridor.has_method("_fix_immediately"):
			corridor._fix_immediately()
