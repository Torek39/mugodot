...
func _check_code(text: String) -> bool:
	var has_while: bool = false
	var has_condition: bool = false
	var has_decrement: bool = false
	...
	for line in text.split("\n"):
		...
		if s.begins_with("while "):
			has_while = true
			if (">" in s and "0" in s) or ("<" in s) or ("==" in s) or ("!=" in s):
				has_condition = true
	...
	return has_while and has_condition and has_decrement and has_function_call

# ==========================
# УБРАТЬ КАМНИ
# ==========================
func _clear_rocks() -> void:
	Global.rocks_cleared = true
	var node2d = _find_node_by_name(get_tree().current_scene, "Node2D")
	if node2d:
		var roks = node2d.get_node_or_null("roks")
		if roks:
			var tween = create_tween()
			tween.tween_property(roks, "position:y", roks.position.y + 55.0, 1)
